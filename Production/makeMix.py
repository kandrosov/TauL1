...
def make_mix(cfg_file, output, n_jobs, job_id):
  ...
  for step_idx, step in enumerate(mix_steps):
    ...
    l1tautower_columns = [
      'hwEtEm', 'hwEtHad', 'hwPt', 'relEta', 'relPhi'
    ]

    for col in l1tautower_columns:
      df_in = df_in.Define(f'L1Tau_tower_{col}',
                           f'GetL1TauTowerVar(L1Tau_pt.size(), L1TauTowers_{col}, L1TauTowers_tauIdx)')
      l1tau_columns.append(f'tower_{col}')
