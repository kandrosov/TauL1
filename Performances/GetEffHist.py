...
def add_prediction(input_idx,var,x_bins, model='model_v4'):
	...
	print(pred.shape, var_den_presel.shape, gen_truth.shape, hw_iso.shape)
	all_var = np.vstack((var_den_presel[:], pred[:,0], gen_truth[:,0], hw_iso[:])).T
	...
	var_num_iso = all_var[condition_iso][:,0]
